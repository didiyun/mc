...
// Get object uploadId info for URL.
func getObjectPartList(urlStr string, isFetchMeta bool, encKeyDB map[string][]prefixSSEPair)(map[int]minio.ObjectPart, error) {
	client, err := newClient(urlStr)
	if err != nil {
		return nil,  errors.New(urlStr)
	}

	return client.GetObjetPartsInfo()
}
