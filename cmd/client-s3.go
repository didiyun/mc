...
import (
	...
	"github.com/didiyun/mc/pkg/httptracer"
...
func (c *s3Client) GetObjetPartsInfo() (partsInfo map[int]minio.ObjectPart, err error) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	bucket, object := c.url2BucketAndObject()
	// Bucket name cannot be empty, stat on URL has no meaning.
	...
	// 获取upload
	content, errs := c.getObjectStat(bucket, object, minio.StatObjectOptions{})
	if errs != nil {
		return nil, errors.New("Get Object Stat is failed")
	}
	uploadId := content.Metadata["X-Didi-Uploadid"]
	if uploadId == "" {
		return nil, errors.New("upload id is nil")
	}

	// 获取分片列表
	return c.api.ListObjectParts(bucket, object, uploadId)
}

func (c *s3Client) ReadAt(b []byte, offset int64) (err error) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	bucket, object := c.url2BucketAndObject()
	// Bucket name cannot be empty, stat on URL has no meaning.
	...
	var opts minio.GetObjectOptions
	reader, err := c.api.GetObject(bucket, object, opts)
	if err != nil {
		return err
	}
